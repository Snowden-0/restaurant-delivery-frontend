...
const RatingCard = ({ orderId, onRatingSubmit, existingRating, existingComment, isSubmitting }) => {
  ...
  const handleSubmit = () => {
    ...
    if ((rating > 0 || comment.trim() !== '') && !hasSubmitted) {
      onRatingSubmit(orderId, rating, comment);
      setHasSubmitted(true);
    }
  };
    ...
    <div className="bg-white rounded-xl shadow-lg p-6 md:p-8 w-full max-w-md mx-auto mt-8 border border-amber-100">
        ...
        {[1, 2, 3, 4, 5].map((starValue) => (
          <Star
            ...
            className={`cursor-pointer transition-colors duration-200 ${
              !hasSubmitted ? 'hover:text-amber-400' : 'opacity-70 cursor-not-allowed'
            }`}
      ...
      <div className="mb-6">
        ...
        <textarea
          ...
          className={`w-full p-3 border rounded-lg focus:ring-amber-500 focus:border-amber-500 transition-all duration-200 ${
            hasSubmitted ? 'bg-gray-50 cursor-not-allowed' : 'border-gray-300'
          }`}
      ...
      {!hasSubmitted ? (
        <button
          ...
          className="w-full bg-amber-500 text-white font-bold py-3 px-4 rounded-lg hover:bg-amber-600 transition-all duration-300 transform hover:scale-105 shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
        >
